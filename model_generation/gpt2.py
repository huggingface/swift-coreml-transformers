...
builder.add_transpose(
	...
	output_name='embeddings_addition_t',
	axes=[1, 0, 2, 3, 4]
)

i = 0

ln_weight = model.h[0].ln_1.weight.data.numpy().reshape((1, 1, 768, 1, 1))
ln_bias = model.h[0].ln_1.bias.data.numpy().reshape((1, 1, 768, 1, 1))
ln_epsilon = model.h[0].ln_1.variance_epsilon
...
conv_1D_bias = model.h[0].attn.c_attn.bias.data.numpy().reshape((1, 1, 2304, 1, 1))
conv_1D_weights = model.h[0].attn.c_attn.weight.data.numpy().transpose().reshape((1, 768, 2304, 1, 1))
...
bias_0 = model.h[0].attn.bias
nd = ns = sequence_length
b = (model.h[0].attn.bias[:, :, ns-nd:ns, :ns]).unsqueeze(0)
...
conv_1D_proj_bias = model.h[0].attn.c_proj.bias.data.numpy().reshape((1, 1, 768, 1, 1))
conv_1D_proj_weights = model.h[0].attn.c_proj.weight.data.numpy().transpose().reshape((1, 768, 768, 1, 1))
...
builder.add_add_broadcastable(
	...
	input_names=["embeddings_addition_t", f"{i}_block_attn_conv_proj"],
	output_name=f"{i}_block_xa_sum"
)

ln_2_weight = model.h[0].ln_2.weight.data.numpy().reshape((1, 1, 768, 1, 1))
ln_2_bias = model.h[0].ln_2.bias.data.numpy().reshape((1, 1, 768, 1, 1))
ln_2_epsilon = model.h[0].ln_2.variance_epsilon
...
mlp_conv_1D_fc_bias = model.h[0].mlp.c_fc.bias.data.numpy().reshape((1, 1, 3072, 1, 1))
mlp_conv_1D_fc_weights = model.h[0].mlp.c_fc.weight.data.numpy().transpose().reshape((1, 768, 3072, 1, 1))
...
mlp_conv_1D_proj_bias = model.h[0].mlp.c_proj.bias.data.numpy().reshape((1, 1, 768, 1, 1))
mlp_conv_1D_proj_weights = model.h[0].mlp.c_proj.weight.data.numpy().transpose().reshape((1, 3072, 768, 1, 1))
